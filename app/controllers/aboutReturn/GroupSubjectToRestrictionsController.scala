...
import nunjucks.{GroupSubjectToRestrictionsTemplate, Renderer}
...
class GroupSubjectToRestrictionsController @Inject()(
  ...
  private def viewHtml(form: Form[Boolean], mode: Mode)(implicit request: Request[_]) = if(isEnabled(UseNunjucks)) {
      renderer.render(GroupSubjectToRestrictionsTemplate, Json.toJsObject(YesNoRadioViewModel(form, mode)))
    } else {
