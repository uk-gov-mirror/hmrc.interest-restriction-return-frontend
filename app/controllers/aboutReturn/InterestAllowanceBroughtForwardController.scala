...
import nunjucks.{InterestAllowanceBroughtForwardTemplate, Renderer}
...
class InterestAllowanceBroughtForwardController @Inject()(
  ...
  private def viewHtml(form: Form[_], mode: Mode)(implicit request: Request[_]) = if(isEnabled(UseNunjucks)) {
    renderer.render(InterestAllowanceBroughtForwardTemplate, Json.toJsObject(BasicFormViewModel(form, mode)))
  } else {
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      viewHtml(fillForm(InterestAllowanceBroughtForwardPage, formProvider()), mode).map(Ok(_))
  }
