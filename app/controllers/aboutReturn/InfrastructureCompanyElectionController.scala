...
class InfrastructureCompanyElectionController @Inject()(override val messagesApi: MessagesApi,
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) { implicit request =>
    Ok(view(fillForm(InfrastructureCompanyElectionPage, formProvider()), mode))
  }
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      formProvider().bindFromRequest().fold(
        ...
        value =>
          saveAndRedirect(InfrastructureCompanyElectionPage, value, mode)
      )
  }
}
