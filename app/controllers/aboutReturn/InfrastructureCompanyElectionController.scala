...
class InfrastructureCompanyElectionController @Inject()(
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      viewHtml(fillForm(InfrastructureCompanyElectionPage, formProvider()), mode).map(Ok(_))
  }
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      formProvider().bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(InfrastructureCompanyElectionPage, value))
            _              <- sessionRepository.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(InfrastructureCompanyElectionPage, mode, updatedAnswers))
      )
  }
}
