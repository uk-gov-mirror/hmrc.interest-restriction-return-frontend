...
class SessionController @Inject()(override val messagesApi: MessagesApi,
  ...
  def set(): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request => {
      val queryParams = request.queryString.map(x => (x._1, x._2.head)).toList
      Redirect(testonly.controllers.routes.SessionController.show()).addingToSession(queryParams: _*)
    }
  }
