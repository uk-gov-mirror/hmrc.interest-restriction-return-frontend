...
sealed trait NetTaxInterestIncomeOrExpense
  ...
  def options(form: Form[_])(implicit messages: Messages): Seq[RadioItem] = values.map {
    value =>
      RadioItem(
        ...
        content = Text(messages(s"netTaxInterestIncomeOrExpense.${value.toString}")),
        checked = form("value").value.contains(value.toString)
      )
  }

  implicit val enumerable: Enumerable[NetTaxInterestIncomeOrExpense] =
