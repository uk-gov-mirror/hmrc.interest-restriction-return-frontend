...
class DisallowedAmountViewSpec extends DecimalViewBehaviours  {

  val messageKeyPrefix = "disallowedAmount"
  val section = Some(messages("section.groupLevelInformation"))
  val form = new DisallowedAmountFormProvider()()

    "DisallowedAmountView" must {

      def applyView(form: Form[_]): HtmlFormat.Appendable = {
          val view = viewFor[DisallowedAmountView](Some(emptyUserAnswers))
      ...
      behave like decimalPage(form, applyView, messageKeyPrefix, routes.DisallowedAmountController.onSubmit(NormalMode).url, section = section)
