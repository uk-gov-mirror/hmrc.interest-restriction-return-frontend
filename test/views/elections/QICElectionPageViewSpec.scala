...
class QICElectionPageViewSpec extends YesNoViewBehaviours  {

  val messageKeyPrefix = "qICElectionPage"
  val section = Some(messages("section.elections"))
  val form = new QICElectionPageFormProvider()()

    "QICElectionPageView" must {

      def applyView(form: Form[_]): HtmlFormat.Appendable = {
        val view = viewFor[QICElectionPageView](Some(emptyUserAnswers))
      ...
      behave like yesNoPage(form, applyView, messageKeyPrefix, routes.QICElectionPageController.onSubmit(NormalMode).url, section = section)
