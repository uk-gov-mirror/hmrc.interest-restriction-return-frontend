...
import pages.testIntPagePage
import play.api.mvc.Call
import play.api.test.Helpers._
import views.html.testIntPageView

class testIntPageControllerSpec extends SpecBase {

  val view = injector.instanceOf[testIntPageView]
  val formProvider = new testIntPageFormProvider()
  val form = formProvider()
  val validAnswer = 0

  def controller(dataRetrieval: DataRetrievalAction = FakeDataRetrievalActionEmptyAnswers) = new testIntPageController(
    ...
    formProvider = new testIntPageFormProvider,
    controllerComponents = messagesControllerComponents,
    view = view
  )

  "testIntPage Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(testIntPagePage, validAnswer).success.value
