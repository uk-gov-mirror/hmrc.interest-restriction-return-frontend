...
class GroupRatioPercentageControllerSpec extends SpecBase with FeatureSwitching with MockDataRetrievalAction {

  val view = injector.instanceOf[GroupRatioPercentageView]
  val formProvider = injector.instanceOf[GroupRatioPercentageFormProvider]
  val form = formProvider()

  val validAnswer = 0

  object Controller extends GroupRatioPercentageController(
  ...
  "GroupRatioPercentage Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set[BigDecimal](GroupRatioPercentagePage, validAnswer).success.value
